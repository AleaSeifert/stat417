...
Over the course of our analysis, we developed many Cox regression models. Through these analyses, we found insignificant evidence to conclude that one or more genders have are significantly different Mini completion times. Similarly, there was insignificant evidence for squares, reaction time or day, any combination of variables (both as main effects and interactions), to conclude that different values of these variables have significantly different Mini completion times. 
...
The LOWESS fit is approximately horizontal, meaning the proportional hazards assumption will not been violated. However, we will investigate this further with a formal test.
...
Testing Proportional Hazards with `cox.zph()` returns a p-value larger than any reasonable value of significance. This indicates that our P.H. assumption is not violated.
